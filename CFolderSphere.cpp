...
{
	if (positioner)
		positioner->recalcPositions(smgr, this, children);
	/*int size=children.size();

	if (!parent)
		recalcChildrenPos(0, 2*core::PI);
	else
	{
		...
		f32 childDiameter=sphereRadius*curScale*sphereScale*2;
...
void folderSphere::setPositioner(IEntryPositioner *pos)
{
	if (positioner!=pos)
		pos->recalcPositions(smgr, this, children);

	entrySphere::setPositioner(pos);
}
